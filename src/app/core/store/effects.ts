...
export class PlayerEffects {
    ...
    isPlayerInDb$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.LOOKUP_PLAYER_DB)
            .do(data => console.log('lookup in fb'))
            .map(toPayload)
            .switchMap(
                params => this.firebaseService.fetchPlayer(params.name, params.region)
                              .take(1)
                              .catch((err, caught) => Observable.of(new PlayerActions.LookupPlayerInFirebaseFail(params)))
                              .map(
                                  player => player['matches'] ? new PlayerActions.LookupPlayerInFirebaseSuccess(player) :
    ...
    playerInDb$: Observable<Action> = this.actions$.ofType(PlayerActions.ActionTypes.LOOKUP_PLAYER_DB_SUCCESS)
                                          ...
                                          .map(toPayload)
                                          .mergeMap(player => Observable.from([
    ...
    lookupPlayerApi$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.SEARCH_PLAYER_API)
            .do(data => console.log('search player in api'))
            .map(toPayload)
            .switchMap(params => this.apiService.getPlayer(params.name, params.region))
            .do(data => console.log(data))
            .map(player => player ? new PlayerActions.LookupPlayerInApiSuccess(player) : new PlayerActions.SearchPlayerFail(false));
    ...
    playerInApi$: Observable<Action> = this.actions$.ofType(PlayerActions.ActionTypes.SEARCH_PLAYER_API_SUCCESS)
                                           ...
                                           .map(player => new PlayerActions.LookupMatchesInApi(player))
                                               ...
                                               @Effect()
    matchesInApi$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.SEARCH_MATCHES_API)
            .do(data => console.log('matches in api?'))
            .map(toPayload)
            .switchMap(
                player => this.apiService.getLatestMatches(player.region, player.name)
                              .map(
    ...
    @Effect()
    updateFirebasMatches$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.SEARCH_MATCHES_API_SUCCESS)
            ...
            .switchMap(params => {
                ...
                return this.firebaseService.addPlayer(player, player.region)
                    ...
                    .concatMap(someval => Observable.from([
                        new PlayerActions.SearchPlayerSuccess(true),
                        new PlayerActions.UpdateFirebaseObservables(player),
    ...
    @Effect()
    markAnApiRequest$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.SEND_TO_REQUEST_QUEUE)
    ...
    @Effect()
    fetchLatestMatchesFromApi$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.HYDRATE_MATCHES)
            ...
            .mergeMap(
                someval => Observable.from([new PlayerActions.SendToRequestQueueDone(true), new PlayerActions.HydrateMatchesDone(true)]));
    ...
    syncUiToFirebase$: Observable<Action> = this.actions$.ofType(PlayerActions.ActionTypes.UPDATE_FIREBASE_OBSERVABLES)
                                                ...
                                                .concatMap(player => Observable.from([
    ...
    @Effect()  // This action will fire every time an update is made to the firebase player and on demand
    updateMatchesFromFirebase$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.UPDATE_MATCHES)
            .map(toPayload)
    ...
    @Effect()  // This action will fire every time an update is made to the firebase player and on demand
    updateMatchCountFromFirebase$: Observable<Action> =
        this.actions$.ofType(PlayerActions.ActionTypes.UPDATE_MATCH_COUNT)
            .map(toPayload)
            .switchMap(params => this.firebaseService.fetchMatchCount(params.name, params.region))
            .concatMap(
                count => Observable.from(
                    [new PlayerActions.UpdateMatchCountDone(count), new PlayerActions.UpdateFirebaseObservablesDone(true)]));
    ...
    pageMatches$: Observable<Action> = this.actions$.ofType(PlayerActions.ActionTypes.GO_TO_MATCH_PAGE)
                                           ...
                                           .withLatestFrom(this.state$)
                                           .map(
                                               ([page, state]) => new PlayerActions.UpdateMatches(
                                                   {name: state.playerName, region: state.region, page: page, limit: state.pageSize}), );

    constructor(
        private actions$: Actions, private firebaseService: FirebaseService, private apiService: ApiService,
