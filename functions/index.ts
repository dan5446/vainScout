...
import {FlatMatch, FlatPlayer, VgApiResponse} from './datatypes';
...
const addNextApiRequest = (event, region, playerName) => {
    event.data.adminRef.root.child(`/players/${region}/${playerName}/matches`).once('value').then((data: admin.database.DataSnapshot) => {
        ...
        console.log(`An api request has been set for ${playerName} : ${
                                                                       region
                                                                     } from 
    ...
    const tokens = [
        'erR5jF19hik:APA91bFwAwBrOX5Qy0CWxPkFWRHpHqycKW1DtY80645MJe0Za_LjZ-_G60SsGVz7W9Rfi2eeFDVdv6NSRPxVwVA-arHMsvdp1mje0QK3OwZQCtqtEFqc-NB-_fjP97xTdAFSHopkEYhj'
    ];
...
const addPlayer = (flatPlayer: FlatPlayer, region: string,
                   event: any) => event.data.adminRef.root.child(`/players/${region}/${flatPlayer.name}`).update({...flatPlayer });
...
const addMatches = (matches: FlatMatch[], playerName: string, region: string, event: any) => {
    ...
    return event.data.adminRef.root.child(`/matches/${region}`)
        .update({...matchDict })
        .then(event.data.adminRef.root.child(`/players/${region}/${playerName}/matches`).update({...matchMetas }));
};

// Executable Firebase Function
export const matchRequestConsumer = functions.database.ref('/apiQueue/{region}/{playerName}').onWrite(event => {
    const region = event.params.region;
    const playerName = event.params.playerName;
    ...
    request(options)
        .then((matches) => {
            ...
            orderedFlatMatches = orderedFlatMatches.sort((a, b) => moment(b.createdAt).valueOf() - moment(a.createdAt).valueOf());
            addMatches(orderedFlatMatches, playerName, region, event)
                .then(event.data.adminRef.root.child(`playerQueue/${region}/${playerName}`).set({ requestedAt: new Date().toISOString() }));
        })
